...
const recommendContent = document.getElementById("recommendContent"); // optional recommendation banner
...
async function showTopAnime() {
  try {
    ...
    startAutoRecommend(); // start 5s recommendation loop
  } catch (err) {
...
function showRandomRecommendation() {
  ...
  recommendContent.innerHTML = `
    <img src="${anime.images.jpg.image_url}" alt="${anime.title}">
    <h3>${anime.title}</h3>
    <p><strong>Rating:</strong> ${anime.score || 'N/A'} • <strong>Episodes:</strong> ${anime.episodes || 'N/A'}</p>
  `;
}
...
function startAutoRecommend() {
  if (recommendContent) {
    ...
    setInterval(showRandomRecommendation, 5000); // every 5s
  }
}
